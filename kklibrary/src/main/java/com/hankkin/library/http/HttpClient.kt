...
object HttpClient {
    ...
    private val mHttpClient by lazy {
        OkHttpClient.Builder()
            ...
            .addInterceptor({ chain -> addParam(chain) })
    ...
    private fun addParam(chain: Interceptor.Chain): Response? {
        val request = chain.request()
        val para = HttpConfig.getParams();
        val newUrl = request.url().newBuilder()
            .addQueryParameter("padAppVer", para!!["padAppVer"].toString())
            .build()
    ...
    private fun addHeader(chain: Interceptor.Chain): Response {
        val request = chain.request()
        val para = HttpConfig.getHeaders();
        val build = request.newBuilder()
            .addHeader("X-Requested-With", "XMLHttpRequest")
            .addHeader("Platform", "Android")
            .addHeader("Authorization", para!!["Authorization"].toString())
            .build()
